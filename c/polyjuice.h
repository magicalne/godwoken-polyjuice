               ...
               gw_context_t *ctx)
{
   ...
   */
  static const uint8_t eth_polyjuice_args_header[7] = {0xff, 0xff, 0xff, 'P', 'O', 'L', 'Y'};
                      ...
                      uint64_t *code_size, uint64_t offset, uint8_t *code)
{
  ...
  if (memcmp(code_hash_seg.ptr, g_script_code_hash, 32) != 0 || *hash_type_seg.ptr != g_script_hash_type
...
int load_globals(gw_context_t *ctx, uint32_t to_id)
{
  ...
  else if (raw_args_seg.size == CONTRACT_ACCOUNT_SCRIPT_ARGS_LEN)
  {
    ...
    if (memcmp(creator_code_hash_seg.ptr, code_hash_seg.ptr, 32) != 0 || *creator_hash_type_seg.ptr != *hash_type_seg.ptr
        /* compare rollup_script_hash */
        || memcmp(creator_raw_args_seg.ptr, raw_args_seg.ptr, 32) != 0 || creator_raw_args_seg.size != CREATOR_SCRIPT_ARGS_LEN)
    {
                    ...
                    bool to_address_is_eoa)
{
  ...
  if (msg->kind == EVMC_CALL && memcmp(msg->sender.bytes, g_tx_origin.bytes, 20) == 0 && to_address_is_eoa)
  {
                      ...
                      struct evmc_result *res)
{
  ...
  {
    ctx, code_data, code_size, from_id, to_id, sender, destination, 0
  };
                   ...
                   const evmc_message *msg_origin, struct evmc_result *res)
{
  ...
  if (memcmp(zero_address.bytes, msg.destination.bytes, 20) != 0)
  {
    ...
    {
      debug_print_int(
          "[handle_message] get to id: load_account_id_by_eth_address failed", ret);
    }
    else
    {
  ...
  {
    debug_print_int(
        "[handle_message] get from id: load_account_id_by_eth_address failed", ret);
    return ret;
  }
